...
* __Several ways to contributed__ to ensure flexibility and simplicity ;
    * *Edit this page* button `r fontawesome::fa("fab fa-github")` on [www.book.utilitr.org](https://www.book.utilitr.org) to use `Github` visual interface (*pull request* is automatic).
...
- __No need to be Git or Github fluent to contribute ;__
- __Contributing help people to discover opensource standards that bring more reliable projects.__


---
# What about `utilitR` tomorrow ?

- `utilitR` keeps being updated to catch up latest `R` evolutions ;
- There are still some missing parts in `utilitR` :
    + How to make nice maps with `R` ?
    + `shiny`
- We need to go further in the harmonization of practices in `R`:
    + A guide of good practices needs to be updated
    + Adapt my latest course on this subject with `Python` (https://ensae-reproductibilite.netlify.app/) to `R` context
