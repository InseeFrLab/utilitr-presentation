...
* Des tests automatiques qui vérifient que les contributions ne comportent pas d'erreur de programmation, et que les exemples sont bien reproductibles (intégration continue) ;
* Des scripts automatiques qui compilent la documentation et déploient le site internet à chaque modification de la branche `master` (déploiement continu) ;
...
+ elle facilite le travail des coordinateurs et des contributeurs ;
+ elle assure un environnement plus reproductible et facilite le repérage des erreurs ;
+ elle permet de se concentrer sur le fond ;
+ elle n'oblige pas à connaître tous les détails de la production de la documentation (on peut ne pas connaître `R` et contribuer).
