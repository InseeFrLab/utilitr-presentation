...
Documentation qui prend plusieurs formes (site internet, et brochure).
Les recommandations sont donc adaptées au contexte de travail des agents du service statistique public
...
## Une organisation efficace au service de la reproductibilité

* Projet entièrement _open source_ (`r fontawesome::fa("fab fa-github")` [`InseeFrLab`](https://github.com/InseeFrLab/utilitR)) ;
* Démarche collaborative impliquant __plus de 15 contributeurs__ :
    + des agents géographiquement dispersés ;
    + des agents issus de plusieurs institutions du SSP.
* Modèle `Wikipedia`:
    + relecture par les pairs ;
    + validation collégiale ;
* Acculturation à des outils favorisant la **reproductibilité** et la **pérennité**, au-delà de `r fontawesome::fa("fab fa-r-project")` :
...
* Une documentation pensée pour le statisticien:
    + Exemples sur [données réelles Insee](https://www.insee.fr) (grâce à [`doremifasol`](https://github.com/InseeFrLab/DoReMIFaSol)) ;
    + Documentation entièrement reproductible 

* Processus de publication à l'état de l'art :
    + Plusieurs _outputs_ (site web, livre PDF) avec les mêmes codes sources
...
<br>
__Le projet `utilitR` est un laboratoire de la manière dont les évolutions récentes de l'écosystème `R` peuvent bénéficier au travail du statisticien__

---
# Contenu de la documentation

COMPLETER SUR LES DEUX GRANDES PARTIES
...
- un site internet ([www.utilitr.org](https://www.utilitr.org)) ;
...
le HTML
    + package `pagedown` pour `.md` -> `.html` -> `.pdf` ;
